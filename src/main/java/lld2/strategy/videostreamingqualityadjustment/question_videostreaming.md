...
- Create three concrete strategy classes, each corresponding to a different video quality (`LOW`, `MEDIUM`, `HIGH`).
...
- Modify the `VideoStreamingManager` class to accept a `QualityAdjustmentStrategy` object in its constructor.
...
- Ensure that you have implemented the Strategy pattern correctly by passing all test cases in the `VideoStreamingManagerTest` class.
- The test cases validate:
    - There are three concrete strategies,
    - The `QualityAdjustmentStrategy` interface has required methods,
    - The `VideoStreamingManager` class is correctly updated to use strategies for quality adjustment.
- Verify that each strategy's `adjust` method correctly adjusts video quality settings based on support.
- Remember to refactor existing code accordingly and ensure all tests pass successfully.
